...
f = open('./mobilite-cesure-choix-post-2a_raw.txt', 'r', encoding='utf-8')
texte = f.read()
f.close()
...
texte = regex.sub("o +",u'',texte)     # enlever les parenthèses et leur contenu

texte = regex.sub(r"\n+", r" ", texte) # supprimer les retours à la ligne dans un paragraphe

texte = regex.sub(r" +", " ", texte) # supprimer les espaces en trop

with open('./mobilite-cesure-choix-post-2a.txt', 'w', encoding='utf-8') as f:
    f.write(texte)
f.close()
