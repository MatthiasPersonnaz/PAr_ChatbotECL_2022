...
batch_size = 32*16   # Batch size for training.
epochs = 2  # Number of epochs to train for.
latent_dim = 64 +32    # Latent dimensionality of the encoding space.
...
#sorties = [[skipgram.wv[u]+[skipgram.wv['<end>']] for u in s] for s in lemmatizedSentencesSpacy]
...
encoder_model = keras.Model(encoder_inputs, state_h)
...
_, state_h_enc = model.layers[2].output  # premier GRU
encoder_model = keras.Model(encoder_inputs, state_h_enc)

decoder_inputs = model.input[1]  # input_2
decoder_state_input_h = keras.Input(shape=(latent_dim,))
decoder_gru = model.layers[3] #deuxieme GRU
